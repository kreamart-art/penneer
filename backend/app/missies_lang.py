...
# key -> (teller, doel, XP, cash). De teller verwijst naar een telling in
...
def zoek(periode_soort: str, key: str, nu: datetime.datetime | None = None):
    ...
    if key not in _kies(pool, aantal, periode):
        return None
    teller, doel, xp, cash = pool[key]
    return {"periode": periode, "teller": teller, "target": doel, "reward": xp, "cash": cash, "van": van, "tot": tot}
